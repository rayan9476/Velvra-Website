...
import { fetchBlog } from "@/app/lib/services";
import { useDispatch, useSelector } from "react-redux";
import {
  setBlogData,
  setError,
  setLoading,
} from "../redux/features/BlogInfoSlice";
...
export default function BlogDetail() {
  ...
  const { blogdata, loading, error } = useSelector((state) => state.blogInfo);
  useEffect(() => {
    const loadBlogs = async () => {
      try {
        dispatch(setLoading());
        const blogData = await fetchBlog(id);
        dispatch(setBlogData(blogData[0]));
      } catch (err) {
        dispatch(
  ...
  return (
    <section className="h-full   pb-8 md:pb-16 lg:pb-20 xl:pb-24 2xl:pb-28 3xl:pb-32 font-[Tenor_Sans] font-normal">
      <figure className="relative flex items-start justify-start w-full h-[40vh] sm:h-[45vh] md:h-[50vh] lg:h-[55vh] xl:h-[80vh] 3xl:h-[85vh] mb-6 md:mb-8 overflow-hidden ">
        <Image
      ...
      <div className="just-for-you  relative mx-auto w-full mb-8 md:mb-10 px-2.5 md:px-4 lg:px-5">
        <Swiper
          ...
          aria-label="Blog image slider"
          aria-roledescription="carousel"
        >
          {blogdata?.slider_images?.map((img, i) => (
            <SwiperSlide
              key={i}
              className="!flex !items-center !justify-center"
              aria-label={`Slide ${i + 1} of ${blogdata.slider_images.length}`}
            >
              <figure className="relative w-full  aspect-[3/4]  max-h-[450px] md:max-h-[600px] md:max-w-[550px]  lg:max-h-[600px]  2xl:max-h-[650px] 2xl:max-w-[600px] 3xl:max-h-[800px] ">
                <Image
                  src={img}
                  alt={`slide-${i}`}
                  fill
